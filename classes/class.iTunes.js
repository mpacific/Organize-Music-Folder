...
module.exports = class iTunesLibrary extends Base {
  ...
  async updateTrackLocations (tracks) {
    for (const track of tracks) {
      track.Location = track.Location.replace(process.env.ITUNES_LIBRARY_OLD_MEDIA_PATH, this.sourcePath)
    }
  }
  ...
  async processiTunesLibrary () {
    ...
    if (process.env.ITUNES_LIBRARY_SEGMENT_PLAYLIST) {
      tracks = await this.processTrackSegments(iTunesLibraryData.Playlists, tracks)
    } else {
      tracks = {
        All: tracks
      }
  ...
  async buildTrackMap () {
    ...
    for (const parentFolder in tracks) {
      ...
      for (const track of folderTracks) {
        const fileInfo = await Path.parse(track.Location.replace('file://', ''))
