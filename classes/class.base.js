...
module.exports = class Base {
  async processTracks() {
    for (const track of this.trackMap) {
      ...
      const artistFolderName = ReplaceSpecialCharacters(
        SanitizeFilename(track.artist, { replacement: "_" })
      );
      const albumFolderName = ReplaceSpecialCharacters(
        SanitizeFilename(track.album, { replacement: "_" })
      );
      const parentFolderName = track.parentFolder
        ? ReplaceSpecialCharacters(
            SanitizeFilename(track.parentFolder, { replacement: "_" })
          )
        : "";
      const fileName = `${
        track.trackNo ? `${track.trackNo}. ` : ""
      }${ReplaceSpecialCharacters(SanitizeFilename(track.title))}${track.ext}`;

      const parentFolderPath = `${this.destPath}${
